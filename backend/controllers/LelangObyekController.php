...
class LelangObyekController extends Controller
{
    ...
    public function actionCreateTanahBangunan()
    {
        $model = new LelangObyek();
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->obyek_lelang = "Sebidang tanah berikut bangunan sesuai";

            $model->save();
            return $this->redirect(['view', 'id' => $model->id]);
        } else {
            return $this->render('create-tanah-bangunan', [
                'model' => $model,
            ]);
        }
    }
    ...
    public function actionAddRl()
    {
        ...
        $connection->createCommand()->update('lelang_obyek',['rl_no'=>$id_rl], ['id' => $id_obyek])->execute();
